...
class PoseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        ori_reg = lo_ori[:, :self.LO_ORI_BIN]
        ori_cls = lo_ori[:, self.LO_ORI_BIN:]
        ...
        centroid_reg = lo_ct[:, :3]
        size_reg = lo_ct[:, 3:]

        return {
            'total3d': {
                'pitch_reg': pitch_reg, 'roll_reg': roll_reg,
                'pitch_cls': pitch_cls, 'roll_cls': roll_cls,
                'ori_reg': ori_reg, 'ori_cls': ori_cls,
                'centroid_reg': centroid_reg, 'size_reg': size_reg
            },
            'afeatures': x
        }
