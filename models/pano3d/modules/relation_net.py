...
class RelationNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        model_config = cfg.config['model']['bdb3d_estimation']
        self.d_k = model_config.get('d_k', 64)
        self.Nr = model_config.get('Nr', 16)

        # branch to estimate geometric weights
        self.fc_g = nn.Linear(cfg.config['data']['g_feature_length'], self.Nr)
        self.threshold = nn.Threshold(1e-6, 1e-6)
        self.softmax = nn.Softmax(dim=1)

        # branch to estimate appearance weights
        self.fc_K = nn.Linear(2048, self.d_k * self.Nr)
        self.fc_Q = nn.Linear(2048, self.d_k * self.Nr)
