...
def clean(args):
    ...
    to_deletes = [p for p in to_deletes
                  if p.split('/')[-3] not in ['pretrained_models', 'detector', 'demo', 'total3d']]
    to_deletes.sort()
...
def build(args):
    ...
    if args.subwork is None or 'py360convert' in args.subwork:
        print("Installing py360convert...")
        py360convert_path = os.path.join(root, 'external', 'py360convert')
        cmd = ' && '.join([
            f"cd {py360convert_path}",
